...
Suggestion = c(" More insights could be indicated as part of the chart title from findings of the visualisation. For example 'Labour Force participation rates sees increase for most age groups from 2010 to 2021'",
               ...
               "% could be indicated, allowing readers to know that is it referring to percentage of age-group.",
               ...
               " Compressing the view in such a manner may confuse the reader and some data reduction or summary could be used such that the full year range is indicated instead.")
...
Critique <-  c("**Unnecessary use of color**: The use of separate colors for each of the age groups may not be necessary given that they are separated by individual column labels. Using of separate colors require the reader to keep track of numerous colors with respect to the age groups.",
             ...
             "**Non-Intuitive age-group arrangement**: Result of arranging the age groups by y-axis values results in age groups mostly arrange from older on the left to younger on the right. Instead should follow the natural order of age groups going from young to old.")
...
- Instead of using a full x-axis with years for each of the age-groups dots are used to show the year to year percentage values without need for axis labels, such that the labels would not be obscured and readers are able to interpret the individual years and participation rate from the dots on the chart.
...
- Full axis labels with units such at readers are not left to interpret what the axis are about.
- Age groups are arrange in natural order so that readers could easily orientate the data and make their own comparison.
...
- For age-groups are presented as a single colour to reduce interpretatoin require, and only used to highlight trends on the charts.
...
Steps <-  c("Using excel, delete comments and unused rows in the data set. In this case rows 1, 2, 10, 11, 18, 19, 26 to 32.",
            ...
            "Next the table is pivoted by to convert the data into long format. This is done by holding down the shift key and while clicking and dragging across all the individual year labelled columns to select them. Then tight clicking the selection and click pivot.",
...
Steps <-  c("Using the data from the previous step, variables 'Age-Group' follow by 'Year' is drag over to the columns shelf. 'Then Labour Force Participation' is dragged over the the rows shelf.",
            ...
            "To format the annotation box, right click the box and select format. Border was change to grey color with solid line and rounded corners as highlighted in orange.",
...
The following steps were used to prepare the data for a graph to indicate difference in labour force participation rate between 2010 and 2021. The same from data would be used without pivoting the data in Tableau, the tidied data worksheet from above steps was renamed as "msrd_res_Data_Double" and the excel file was save as a different name.

```{r}

Steps <-  c("Copy of data is added into tableau as another data source by clicking the data source icon at the top of the pane as selecting new data source option.",
...
Steps <-  c("Start with adding a new worksheet, by clicking the new sheet icon.",
            ...
            "Drag and drop a 'Difference' calculated field to colors. To add colors to the plot to indicate positive and negative difference.",
...
Steps <-  c("Start a new dashboard by clicking add dashboard icon at the bottom of window highlighted in orange.",
            ...
            "Finally hide both 'Trend' and 'Difference' worksheets before publishing the workbook."
            )
...
aesthetics <- data.frame(Item = c(1,2,3,4),
                         ...
                         Takeaway = c("Comparing the labour participation rate between 2010 to 2021 majority of the age groups show an increase in particition rate over the years. Indicating that more individuals remain work even to later years, which could possibly be attributed to raising cost of living or changes in family roles to support both elders and childern.",
                                         ...
                                         "From 2010 to 2021, the only age group to see a drop in labour participation rate is the 20 to 24 group. Seeing a drop by -3.4 percentage points. This is likely driven by changes in education demands and more youths seeking degrees before starting their first job.",
                                      "Both this age-groups saw a sharp drop in labour participation rate in 2020. This is likely driven by Covid-19 pandemic restrictions on F&B service which are typically staffed by part-time workers. It could also be driven by difficult of fresh graduates seeking employement during this period, while companies are cutting cost. However the overall participation rate increase up in 2021 as youths finding alternate employment opportunities."))
library(knitr)
kable(aesthetics,align = "cll")%>%
kable_styling(full_width = F) %>%
column_spec(1, bold = T) %>%
