...
* The numerous Age groups and planning area if presented as individual as is creates a lot of data points in the visualization which would require additional mental load from the viewer to keep track of the information.
...
pop2021_grouped <- pop2021_grouped %>% mutate(Size = case_when(Sex == "Males" ~ 0-n,
                                            TRUE ~ n))
```
...
```{r rename, echo = TRUE}
pop2021_grouped <- rename(pop2021_grouped, Planning_Area = PA)
```
...
ggplot(pop2021_grouped, aes(x = Planning_Area, y = Size, color = Age_Groups, shape = Sex)) + 
  geom_point() +
...
p <- ggplot(pop2021_grouped, aes(x = Planning_Area, 
                                 y = Size, 
                                 color = Age_Groups, 
                                 frame = Time)) + 
  geom_point() +
